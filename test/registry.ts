...
const parseCong = (value: string) => parseUnits(value, 8 + 18);
...
const setupFixture = deployments.createFixture(async ({ ethers, deployments }) => {
    ...
    for (const user of users) {
        ...
        await cong.connect(deployer).mint(user.address, parseCong("100"));
        ...
        await cong.connect(user).approve(registry.address, parseCong("100"));
    }
...
describe("KeeperRegistry", function () {
    ...
    describe("punishKeeper()", function () {
        ...
        it("should punish cong keeper and confiscate the rest", async function () {
            await registry.connect(users[0]).addKeeper(cong.address, parseCong("10"));
        ...
        it("should punish cong & non-cong keepers and confiscate the rest", async function () {
            ...
            await registry.connect(users[2]).addKeeper(cong.address, parseCong("10"));
